...
if( isset($_POST) && !empty($_POST) && isset($_REQUEST['action'])=='requestReset' ){
    ...
    if( $count == 1 ) {
	    ...
	    $secure = md5( $email );
	    $link = "<a href='https://www.samplerun.quantumscientific.co.ke/reset.php?key=". $email ."'>Click To Reset password</a>";
	    ...
	    if( $mail->Send() ) {
	        ...
	        echo "<script type='text/javascript'> document.location = 'requestPasswordReset.php?status=$message'; </script>";
	    } else{
        ...
        echo "<script type='text/javascript'> document.location = 'requestPasswordReset.php?status=$message'; </script>";
        //header( "location: requestPasswordReset.php?status=$message" );
    }//End if-count
